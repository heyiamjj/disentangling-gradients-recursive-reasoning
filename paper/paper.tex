...
\author{
  Jatin Jani\thanks{Code: \url{https://github.com/heyiamjj/disentangling-gradients-recursive-reasoning}}\thanks{Models: \url{https://huggingface.co/heyiamjj/disentangling-gradients-recursive-reasoning}} \\
...
\begin{abstract}
Recent work on recursive reasoning has produced two competing architectures: the Hierarchical Reasoning Model (HRM) and Tiny Recursive Models (TRM), which differ dramatically in performance, with 55.0\% versus 87.4\% exact accuracy on Sudoku-Extreme. However, TRM changed both the architecture (dual-network hierarchy $\rightarrow$ single flat network) \emph{and} the gradient method (1-step approximation $\rightarrow$ full backpropagation through time) simultaneously, making it impossible to attribute performance differences to either factor in isolation. In this work, we disentangle these two variables by running TRM's flat architecture with HRM's 1-step gradient approximation. Across an identical training setup (384 hidden dimensions, 2 layers, 10,000 epochs, 1,000 Sudoku examples), we find that exact accuracy collapses from 18.9\% (full BPTT) to 2.2\% (1-step gradient), an 8.6$\times$ reduction. Token-level accuracy drops more modestly from 71.6\% to 65.9\%. Our results demonstrate that an 8.6$\times$ gap produced by varying only the gradient method on identical architecture reveals that TRM's advantage over HRM stems not solely from architecture, but the gradient method also played a critical role in driving its performance. Future work on recursive reasoning must control for gradient method when comparing architectures.
\end{abstract}
...
Either change could explain the performance improvement, or both. To our knowledge, no prior work has systematically isolated these two variables, and the community has subsequently attributed TRM's success to its architectural simplification~\cite{arcprize2025}.
...
Our key finding is that gradient quality dominates: the 1-step gradient approximation reduces exact accuracy by 8.6$\times$ (from 18.9\% to 2.2\%), while token-level accuracy drops by 8.0\% (71.6\% to 65.9\%). The flat architecture \emph{can} learn, but the crippled gradient signal prevents it from learning coherent, complete solutions.
...
\begin{itemize}
    ...
    \item We demonstrate that gradient method alone can produce an order of magnitude difference (8.6$\times$ gap, Table~\ref{tab:results}) in recursive reasoning performance, confounding direct architecture comparisons.
...
\begin{table}[t]
\centering
\caption{The confound in the prior literature. HRM and TRM differ along two dimensions simultaneously, making direct comparison invalid. The community compared HRM (top-left, 55.0\%) to TRM (bottom-right, 87.4\%) and attributed the gap to architecture, ignoring the gradient confound.}
\label{tab:confound}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[t]
\centering
\caption{Results of our controlled experiment. Both models use the same flat TRM architecture and identical training configuration. The only difference is the gradient method. Each cell shows \textit{token accuracy~\% / exact accuracy~\%} (exact in bold). The $2\times2$ matrix now has three of four cells filled; the left column (HRM architecture) is left for future work.}
\label{tab:results}
\begin{tabular}{lccc}
\toprule
...
Table~\ref{tab:results} presents our main results using the $2\times2$ factorial framework. The full BPTT model achieves 18.9\% exact accuracy, while the 1-step gradient model achieves only 2.2\%, an 8.6$\times$ reduction. Token-level accuracy drops from 71.6\% to 65.9\%, a decrease of 5.7 percentage points (8.0\% relative). This pattern suggests that the 1-step gradient approximation is sufficient for learning local patterns (individual cell predictions) but fundamentally prevents the model from learning the global constraints needed for complete puzzle solutions.
...
Our results establish that gradient quality is also a dominant factor in recursive reasoning performance, not architecture alone. The flat TRM architecture \emph{is} capable of learning to solve Sudoku puzzles, reaching 18.9\% exact accuracy, when given proper gradient signals. The same architecture crippled by the 1-step approximation achieves only 2.2\%, barely above the level achievable by learning token-level patterns alone.

This finding has several implications. First, it suggests that the community's attribution of TRM's success to only architecture simplification~\cite{arcprize2025} was premature; the results were confounded by the simultaneous change in gradient method. Second, the 8.6$\times$ collapse in exact accuracy occurs despite token-level accuracy dropping only modestly (71.6\% $\rightarrow$ 65.9\%). The 1-step approximation preserves the model's ability to predict individual cells correctly, but cripples its ability to coordinate them into globally consistent solutions. Gradient quality, not architectural capacity, is what enables the model to satisfy long-range constraints. Third, it establishes a clean experimental framework for future work: the $2\times2$ factorial design (Table~\ref{tab:results}) should be the default when comparing recursive architectures.

Our work has several limitations. We evaluate on a single task (Sudoku-Extreme) with a moderate model size (384 hidden dimensions, 2 layers). The relative importance of gradient quality vs. architecture may differ at larger scales or on tasks requiring different reasoning strategies (e.g., maze pathfinding, abstract reasoning on ARC-AGI). Additionally, our full BPTT model achieves 18.9\% exact accuracy, substantially below TRM's reported 87.4\%. This is expected given our reduced training budget (10,000 vs. 60,000 epochs) and model size (384 vs. 512 hidden dimensions, 2 vs. 4 layers). Scaling to match TRM's full configuration would likely close this gap, but is left for future work.
...
We have demonstrated that architecture alone is not the primary factor explaining TRM's performance advantage over HRM; the gradient method also played an important role. By running a controlled experiment that isolates the gradient method, we show that the 1-step gradient approximation reduces exact accuracy from 18.9\% to 2.2\%, an 8.6$\times$ gap, while the flat architecture remains identical. This resolves a confound that has gone unaddressed in the recursive reasoning literature and establishes a clean experimental paradigm for future comparisons. The $2\times2$ factorial matrix provides a roadmap: our work fills the top-right and bottom-right cells (flat architecture at both gradient methods); the remaining task is evaluating the left column (hierarchical architecture at both gradient methods).
